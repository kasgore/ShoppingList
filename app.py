...
def format_quantity(qty: float) -> str:
    ...
    if qty <= 0:
        return ""
    # Try to render common fractions for cooking-friendly numbers.
    whole = int(qty)
    frac = qty - whole
    if abs(frac) < 0.01:
        return str(whole)
    # Snap to nearest 1/8 for nicer display when very close.
    eighths = round(frac * 8)
    if abs(frac - eighths / 8) < 0.02 and eighths in {1, 2, 3, 4, 5, 6, 7}:
        f = Fraction(eighths, 8)
        if whole:
